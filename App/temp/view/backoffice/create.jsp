...
<%
  ...
  String[] champs = {"date_depart", "date_arrivee", "delai_reservation_heures","delai_annulation_heures","id_aeroport_arrivee","id_aeroport_depart","id_avion"};
  for (String champ : champs) {
    List<String> erreurs = (List<String>) request.getAttribute("error_vol." + champ);
  ...
  if(hasErrors){
    pdDateDepart = (String) request.getAttribute("vol.date_depart");
    pdDateArrivee = (String) request.getAttribute("vol.date_arrivee"); 
    pdAeroportDepart = (String) request.getAttribute("vol.id_aeroport_depart");
    pdAeroportArrivee = (String) request.getAttribute("vol.id_aeroport_arrivee");
    pdAvion = (String) request.getAttribute("vol.id_avion");
    pdReservation = (String) request.getAttribute("vol.delai_reservation_heures");
    pdAnnulation = (String) request.getAttribute("vol.delai_annulation_heures");
  }

  List<String> erreursDateDepart = (List<String>) request.getAttribute("error_vol.date_depart");
  List<String> erreursDateArrivee = (List<String>) request.getAttribute("error_vol.date_arrivee");
  List<String> erreursAeroportDepart = (List<String>) request.getAttribute("error_vol.id_aeroport_depart");
  List<String> erreursAeroportArrivee = (List<String>) request.getAttribute("error_vol.id_aeroport_arrivee");
  List<String> erreursAvion = (List<String>) request.getAttribute("error_vol.id_avion");
  List<String> erreursReservation = (List<String>) request.getAttribute("error_vol.delai_reservation_heures");
  List<String> erreursAnnulation = (List<String>) request.getAttribute("error_vol.delai_annulation_heures");

  Boolean valider = false;
  if (request.getAttribute("valider") != null) {
      valider = (Boolean) request.getAttribute("valider");
  }
  ...
  <body>
            ...
            <form class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800" method="get" action="<%= baseUrl %>/backOffice/vol">
                ...
                <div class="flex-perso-btn">
                    <label class="block mt-4 text-sm">
                      ...
                      <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="vol.id_aeroport_depart">
                          <option value="">Sélectionnez un aéroport</option>
                          <% for(int i=0;i<listeAeroport.size();i++) { %>
                          <option value="<%= listeAeroport.elementAt(i).getId() %>" <%= (pdAeroportDepart != null && pdAeroportDepart.equals(String.valueOf(listeAeroport.elementAt(i).getId()))) ? "selected" : "" %>>
                              <%= listeAeroport.elementAt(i).getVille() %> (<%= listeAeroport.elementAt(i).getCode_iata() %>)
                          </option>
                          <% } %>
                      </select>
                      <% if (erreursAeroportDepart != null && !erreursAeroportDepart.isEmpty()) { %>
                        <span class="text-xs text-red-600 dark:text-red-400">
                          <% for (String erreur : erreursAeroportDepart) { %>
                            <%= erreur %>
                          <% } %>
                        </span>
                      <% } %>
                    ...
                    <label class="block mt-4 text-sm">
                      ...
                      <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="vol.id_avion">
                        <option value="">Sélectionnez une avion</option>
                        <% for(int i=0;i<listeAvion.size();i++) { %>
                        <option value="<%= listeAvion.elementAt(i).getId() %>"  <%= (pdAvion != null && pdAvion.equals(String.valueOf(listeAvion.elementAt(i).getId()))) ? "selected" : "" %>>
                            <%= listeAvion.elementAt(i).getModele() %> (<%= listeAvion.elementAt(i).getCode_avion() %>)
                        </option>
                        <% } %>
                      </select>
                      <% if (erreursAvion != null && !erreursAvion.isEmpty()) { %>
                        <span class="text-xs text-red-600 dark:text-red-400">
                          <% for (String erreur : erreursAvion) { %>
                            <%= erreur %>
                          <% } %>
                        </span>
                      <% } %>
                    ...
                    <label class="block mt-4 text-sm">
                      ...
                      <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="vol.id_aeroport_arrivee">
                        <option value="">Sélectionnez un aéroport</option>
                        <% for(int i=0;i<listeAeroport.size();i++) { %>
                        <option value="<%= listeAeroport.elementAt(i).getId() %>" <%= (pdAeroportArrivee != null && pdAeroportArrivee.equals(String.valueOf(listeAeroport.elementAt(i).getId()))) ? "selected" : "" %>>
                            <%= listeAeroport.elementAt(i).getVille() %> (<%= listeAeroport.elementAt(i).getCode_iata() %>)
                        </option>
                        <% } %>
                      </select>
                      <% if (erreursAeroportArrivee != null && !erreursAeroportArrivee.isEmpty()) { %>
                        <span class="text-xs text-red-600 dark:text-red-400">
                          <% for (String erreur : erreursAeroportArrivee) { %>
                            <%= erreur %>
                          <% } %>
                        </span>
                      <% } %>
                ...
                <div class="flex-perso">
                    <label class="block text-sm">
                      <span class="text-gray-700 dark:text-gray-400">Temps limite pour réserver</span>
                      <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" type="text" name="vol.delai_reservation_heures" value="<%=pdReservation %>"/>
                      <% if (erreursReservation != null && !erreursReservation.isEmpty()) { %>
                        <span class="text-xs text-red-600 dark:text-red-400">
                          <% for (String erreur : erreursReservation) { %>
                            <%= erreur %>
                          <% } %>
                        </span>
                      <% } %>
                    ...
                    <label class="block text-sm">
                      <span class="text-gray-700 dark:text-gray-400">Temps limite pour annuler </span>
                      <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" type="text" name="vol.delai_annulation_heures" value="<%=pdAnnulation %>"/>
                      <% if (erreursAnnulation != null && !erreursAnnulation.isEmpty()) { %>
                        <span class="text-xs text-red-600 dark:text-red-400">
                          <% for (String erreur : erreursAnnulation) { %>
                            <%= erreur %>
                          <% } %>
                        </span>
                      <% } %>
    ...
    <script>
      ...
      document.getElementById("customModalClose").onclick = function () {
        ...
        setTimeout(function () {
          window.location.href = "<%= baseUrl %>/backOffice/crudVol";
        }, 300);
      };
